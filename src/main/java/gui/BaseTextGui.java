...
public class BaseTextGui 
{
	...
	private final int enymyHero=1;
	private final int enymyMinion=2;
	...
	{
		System.out.println("¯ycie przeciwnika "+game.getEnemyOf(game.getActiveHero()).getHealth()+" mana przeciwnika "+game.getEnemyOf(game.getActiveHero()).getMana());
		System.out.println("Karty w rêce przeciwnika");
		...
		System.out.println("Twoje ¿ycie "+game.getActiveHero().getHealth()+" twoja mana  "+game.getActiveHero().getMana());
		System.out.println("Karty w rêce");
	...
	{
		System.out.println("Gra rozpoczêta");
	...
	public void play()
	{
		...
		if(game.getWinner()==game.getFirstHero()) //celowe porównanie referencji 
	...
	{
		System.out.println("1.U¿yj czaru 2.Po³ó¿ kartê 3.U¿yj karty ze sto³u 4.Skoñcz turê ");
		...
		{
			System.out.println("Ruch nie mo¿liwy do wykonania");
		}
	}
	...
	private Move prepareMove(int move)
	{
		switch(move)
		{
		case useSpell: return prepareSpell();
		case putCard: return preparePutCard();
		case attack: return prepareAttack();
		case end: return prepareEndRound();
	...
	{
		System.out.println("Wybierz kartê");
		...
		else
		{
			...
			switch(target)
			{
			case enymyHero: return new AttackHero(minion, game.getActiveHero().getBoard(),game.getEnemyOf(game.getActiveHero()));
			case enymyMinion: 
		...
		if(game.getActiveHero().getHand().get(spell) instanceof Spell)
		{
			...
			else
			{
				...
				if(game.getActiveHero().getHand().get(spell) instanceof HealingTouch)
				{
					...
					switch(target)
					{
					case selfHero: return new UseSpell(spell, game.getActiveHero(), game.getEnemyOf(game.getActiveHero()), null, game.getActiveHero());
					case selfMinion: 
						System.out.println("Wybierz miniona");
						int minion=keyboard.nextInt();
						if(minion<game.getActiveHero().getBoard().size())
				...
				if(game.getActiveHero().getHand().get(spell) instanceof Fireball)
				{
					...
					switch(target)
					{
					case enymyHero: return new UseSpell(spell, game.getActiveHero(), game.getEnemyOf(game.getActiveHero()), null,game.getEnemyOf(game.getActiveHero()));
					case enymyMinion: 
						System.out.println("Wybierz miniona");
						int minion=keyboard.nextInt();
						if(minion<game.getEnemyOf(game.getActiveHero()).getBoard().size())
